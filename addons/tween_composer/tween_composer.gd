...
func _compose_tween() -> void:
	...
	for tw_step in tw_steps:
		...
		if tw_step == null:
			push_error(
				tween_sequence.tween_steps.resource_name + ": Collection contains empty steps."
			)
			return
	...
	if duration_ratio_total <= 0:
		push_warning(
			tween_sequence.tween_steps.resource_name
			+ ": Total duration ratio = 0. Using 1.0 to avoid division by zero. It's likely that all steps are set to parallel."
		)
	...
	for tw_step in tw_steps:
		...
		if _initial_values.has(tw_step.property_name) == false: # Avoid duplicates, only get first value
			_initial_values[tw_step.property_name] = parent_object.get_indexed(
				tw_step.property_name
			)
		...
		if parent_object is Node3D and tw_step.tween_property == tw_step.TweenOptions.MODULATE:
			push_error(
				tween_sequence.tween_steps.resource_name
				+ ": Node3D does not support 'modulate'. Use 'Other' to target a material property."
			)
			continue
		elif (
			(parent_object is CollisionObject2D or parent_object is CollisionObject3D)
			and tw_step.tween_property == tw_step.TweenOptions.SCALE
		):
			push_error(
				tween_sequence.tween_steps.resource_name
				+ ": Changes to the Scale property in PhysicsBody objects may lead to unexpected results or even be overridden"
			)
		...
		if parent_object is Node2D or parent_object is Control:
			...
			if (
				tw_step.tween_property == tw_step.TweenOptions.ROTATION
				and target_value_formatted is Vector3
			):
				target_value_formatted = target_value_formatted.x
			# Transform  Vector3 to Vector2 if 2D
			elif (
				(tw_step.tween_property == tw_step.TweenOptions.POSITION
				or tw_step.tween_property == tw_step.TweenOptions.SCALE)
				and target_value_formatted is Vector3
			):
				target_value_formatted = Vector2(target_value_formatted.x, target_value_formatted.y)

		# Constructing the tween property
		var tw_property = tween.tween_property(
			parent_object,
			tw_step.property_name,
			target_value_formatted,
			tween_sequence.tween_duration * (tw_step.duration_ratio / duration_ratio_total),
		)
		if is_relative:
		...
		if tw_step.duration_delay > 0.0:
			if is_relative:
				push_error(
					tween_sequence.tween_steps.resource_name
					+ ": TweenComposer currently doesn't support the combo of relative + parallel + delay"
				)
			else:
				tw_property.set_delay(
					tween_sequence.tween_duration * (tw_step.duration_delay / duration_ratio_total)
				)

		for trigger in tw_step.send_triggers:
			tween.tween_callback(emit_signal.bind("trigger_fired", trigger))
...
func _is_tween_config_valid() -> bool:
	...
	if tween_sequence.tween_steps == null:
		push_error(
			str(parent_object.name) + ": TweenComposer must have a TweenConfigCollection file!"
		)
		return false
	elif tween_sequence.tween_steps.step_collection.size() == 0:
		push_error(
			str(parent_object.name) + ": " + str(tween_sequence.tween_steps.resource_name)
			+ "Configuration is empty (no tween steps set)!"
		)
		return false
	else:
		return true
...
func _resolve_expression(tw_step: TweenStepItem) -> Variant:
	...
	if text.is_empty():
		push_error(
			tween_sequence.tween_steps.resource_name + " / "
			+ tw_step.step_name + ": Expression is empty!"
		)
	...
	if expression.has_execute_failed():
		push_error(
			tween_sequence.tween_steps.resource_name + " / "
			+ tw_step.step_name + ": Expression execution failed!"
		)
		return tw_step.target_value # Fallback to default target value in step
...
## Loads a new [TweenSequence] resource and starts it WITHOUT resetting the
## parent's properties first. Unlike [method load_tween_sequence_and_start],
## this preserves whatever mid-animation state the parent is currently in,
## so the new tween animates smoothly from the current value instead of
## snapping back to the previous sequence's initial values.
func load_tween_sequence_and_start_from_current(new_resource: TweenSequence) -> void:
	tween_sequence = new_resource
	_compose_tween()
	play_tween()
