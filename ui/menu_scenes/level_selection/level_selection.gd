...
func _ready():
	...
	go_back = DogCommandBuilder.new().go_till_hits_a_wall(dog_speed, DIR.LEFT).build()
	start_level = DogCommandBuilder.new().go_till_hits_a_wall(dog_speed, DIR.RIGHT).build()
